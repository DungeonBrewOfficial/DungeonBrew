/* Hoof & Rein — stat block UI.
 *
 * Renders the live stat block computed by horse-engine.js. Two components:
 *   <CompactStats block />        — single-line summary
 *   <StatBlock    block log? />   — full 5e-style block
 */
  ...
  return (
    <div className="cc-sb-header">
      <p className="cc-sb-type">
        <em>{block.label} — {block.size} {block.type}, {block.alignment}</em>
      </p>
...
function AbilityTable({ block }) {
  return (
    <table className="cc-sb-ability">
      <thead>
        <tr>{SB_ABILITIES.map(a => <th key={a}>{SB_ABILITY_LABEL[a]}</th>)}</tr>
      </thead>
      <tbody>
        <tr className="cc-sb-ability-score">
          {SB_ABILITIES.map(a => <td key={a}>{block.abilities[a]}</td>)}
        </tr>
        <tr className="cc-sb-ability-mod">
          {SB_ABILITIES.map(a => <td key={a}>{SB_SIGN(block.abilityMods[a])}</td>)}
        </tr>
        <tr className="cc-sb-ability-save">
          {SB_ABILITIES.map(a => {
            const save = (block.saves || []).find(s => s.ability === a);
            return <td key={a}>{SB_SIGN(save ? save.bonus : block.abilityMods[a])}</td>;
          })}
        </tr>
      </tbody>
      <tfoot>
...
function MetaLines({ block }) {
  const saves = (block.saves || []).map(s => `${SB_ABILITY_LABEL[s.ability]} ${SB_SIGN(s.bonus)}`).join(", ");
  ...
  return (
    <div className="cc-sb-meta">
      {saves && (<p><span className="cc-sb-meta-k">Saves</span> {saves}</p>)}
      {skills && (<p><span className="cc-sb-meta-k">Skills</span> {skills}</p>)}
...
function ActionList({ block }) {
  const atks = block.attacksRendered || [];
  ...
  return (
    <section className="cc-sb-section">
      ...
      <div className="cc-sb-section-body">
        {atks.map((a, i) => (
          <p key={i}><strong><em>{a.name}.</em></strong> {a.text}</p>
        ))}
      </div>
    </section>
  );
...
function ModifierLog({ log }) {
  ...
  const total = log.reduce((n, x) => n + x.changes.length, 0);
  return (
    <details className="cc-sb-modlog" open>
      <summary>
        ...
        <span className="cc-sb-modlog-count">{total} change{total === 1 ? "" : "s"}</span>
      </summary>
