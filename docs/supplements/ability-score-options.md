...
<a href="../downloads/ability_score_options.pdf" download class="md-button">Download Ability Score Options</a>
...
All ability scores start at 8. The player is given a budget of points to spend on increasing these scores, with the default usually being 27 points for an Elite Array. For scores of 13 or lower, the cost is on a 1-for-1 basis, with the cost increasing for scores of 14 or higher.
...
A maximum ability score can be set (typically 15) to prevent players from purchasing scores that are too high. The point budget can be configured to create different power levels: 15 points for an Average Array, 27 for Elite, 39 for Heroic, and 51 for Mythic. To reintroduce randomness, the point budget itself can be determined by a dice roll.
...
- **Design Philosophy:** This is the most deterministic method, eliminating all randomness from score generation. Every player begins with the same resources. 
- **Mechanical Profile:** The provided arrays are pre-balanced based on tier: Average (PBE 15), Elite (PBE 27), Heroic (PBE 39), and Mythic (PBE 51). Unlike Point Buy, these static scores prevent players from creating extreme statistical builds, which helps to reduce min-maxing. 
- **Impact on Gameplay:** This method is the fastest way to create a character and guarantees equality, but not equity. The arrays can be restrictive for MAD classes, while being highly effective for SAD classes.
...
This method requires rolling a set of three six-sided dice (3d6) for each ability score. The results are recorded in the order they are rolled and cannot be reassigned.

### Alternatives

The player may assign the six rolled scores to their abilities as they choose. The player rolls between 7 and 12 sets of dice and uses the six highest resulting scores for their array, assigning as they choose. Each player at the table rolls one full array, and any player may choose from the resulting pool of arrays for their character, either in order or assigned as they choose.

### Analysis

- **Why use it?** This method prioritizes random character discovery over player control. It is ideal for gritty or low-power campaigns, as its randomness can create challenging and memorable character flaws. 
- **Outputs:** The base method typically produces Average Arrays (PBE ~15). The alternatives, by allowing players to select for higher scores, introduce the possibility of Elite or Heroic arrays. 
- **When to use it:** Use the base method for a classic, old-school experience where players must adapt to the character they are given. Use the alternatives when you want the feel of rolling dice but want to give players more control and reduce the risk of creating an unviable character.
...
- **Why use it?** This method is designed to give players control over guaranteeing a high prime requisite. It allows players to decide between guaranteed values and gambling for their character's array. 
- **Outputs:** The outputs depend on player choice, but any build that includes 2 or 3 point costs will more likely result in a Heroic Array. Relying solely on the 1 point cost will likely result in an Elite Array. 
- **When to use it:** Use this method when you want to include random generation with some player control over outputs. It's a good middle ground for players who dislike pure randomness but don't want a fully deterministic system.
